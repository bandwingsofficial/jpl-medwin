...
export function HomeBrands() { 
  ...
  /* 
   * ONLY display the priority brands.
   *
   * The brands are selected in the exact order of PRIORITY_BRANDS.
   * Any other brand from the API is completely excluded.
   */ 
  const priorityBrands = PRIORITY_BRANDS.flatMap( 
    (priorityBrand) => { 
      const aliases = 
        PRIORITY_BRAND_ALIASES[priorityBrand] ?? [ 
          priorityBrand, 
        ]; 
      ...
      const matchedBrand = (brands ?? []).find( 
        (brand) => { 
          const normalizedName = normalizeBrand( 
            brand.name, 
          ); 
 
          const normalizedSlug = normalizeBrand( 
            brand.slug, 
          ); 
          ...
          <div className="flex animate-marquee-left gap-4 py-1"> 
            {[ 
              ...
              <Link 
                ...
                className="shrink-0" 
              > 
                <div className="box-border flex h-24 w-24 items-center justify-center rounded-2xl border border-solid border-slate-200 bg-white p-4 shadow-sm transition hover:border-teal-500 hover:bg-white hover:shadow-lg"> 
                  {typeof brand.imageUrl === 'string' && 
                  brand.imageUrl.trim() !== '' ? ( 
                    <Image 
                      src={brand.imageUrl} 
