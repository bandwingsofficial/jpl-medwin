...
export function RelatedProducts({
  ...
  const categoryProductsList =
    extractProducts(categoryInfiniteData);

  const totalCatalogList =
    extractProducts(fallbackInfiniteData);
  ...
  const backupFiltered =
    totalCatalogList.filter(
      (product) =>
        product &&
        product.id !== currentProductId &&
        !relatedFiltered.some(
          (r) => r.id === product.id
        )
    );
  ...
  // Items matching the exact same category context
  const discoverMoreProducts =
    relatedFiltered.slice(0, 6);

  // Remaining general backup pool items
  const remainingPool = backupFiltered;

  // Row 2: Trending Selections
  const trendingProducts =
    remainingPool.slice(0, 6);

  // Row 3: Customer Favorites
  const favoriteProducts =
    remainingPool.slice(6, 12);

  // Row 4: Explore Our Catalog
  // Shows up to 100 complete inventory items
