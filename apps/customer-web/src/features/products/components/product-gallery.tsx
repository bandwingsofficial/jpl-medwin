...
const [zoomPosition, setZoomPosition] =
 ...
 const getZoomBackgroundStyle = () => {
  ...
  /*
   * The zoom preview must follow the EXACT point
   * where the mouse is hovering.
   *
   * We calculate the rendered image dimensions
   * inside the circular zoom lens and then move
   * the background using pixel coordinates.
   */
  ...
  let baseWidth = lensSize;
  let baseHeight = lensSize;

  /*
   * Same object-contain behaviour.
   * The complete image fits inside the lens.
   */
  if (imageRatio > 1) {
    baseHeight =
      lensSize / imageRatio;
  ...
  const zoomedHeight =
    baseHeight * zoomLevel;

  /*
   * Convert cursor position into the exact
   * point inside the zoomed image.
   */
  const cursorX =
    zoomPosition.x * zoomedWidth;

  const cursorY =
    zoomPosition.y * zoomedHeight;

  /*
   * Center the hovered point inside the lens.
   */
  let backgroundX =
    lensSize / 2 - cursorX;
  ...
  /*
   * Limit movement so that no empty area
   * appears around the image.
   */
  const minX =
    Math.min(0, lensSize - zoomedWidth);

  const minY =
    Math.min(0, lensSize - zoomedHeight);

  backgroundX =
    Math.min(
      0,
      Math.max(minX, backgroundX)
    );

  backgroundY =
    Math.min(
      0,
      Math.max(minY, backgroundY)
    );

  /*
   * Center the image on an axis where
   * it is smaller than the zoom lens.
   */
  if (zoomedWidth < lensSize) {
    backgroundX =
      (lensSize - zoomedWidth) / 2;
  }

  if (zoomedHeight < lensSize) {
    backgroundY =
      (lensSize - zoomedHeight) / 2;
  }
  ...
  /*
   |----------------------------------------------------------------------
   | EFFECTIVE PER-AXIS ZOOM
   |----------------------------------------------------------------------
   | The lens box is a fixed 340x340 square, but getZoomBackgroundSize()
   | only sets ONE axis explicitly to `zoomLevel * 100%` and leaves the
   | other as `auto` so the image keeps its aspect ratio. That means the
   | "auto" axis is NOT actually scaled by `zoomLevel` — its real scale
   | factor depends on the image's aspect ratio. Feeding the raw
   | `zoomLevel` into getZoomBackgroundPosition() for BOTH axes (as
   | before) makes the position math wrong on whichever axis is "auto",
   | which is what made the preview feel like it was clamped near the
   | center for any non-square product image. This derives the true
   | per-axis zoom so both x and y track the cursor correctly.
   */
