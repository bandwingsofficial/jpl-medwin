...
export class OrderMapper {
  ...
  static toDomain(p: PrismaOrderWithAddresses): Order {
    const order = new Order(
      ...
      Number(p.overweightDeliveryCharge ?? 0),
      ...
      p.gstNumber ?? undefined,

      p.adminNote ?? undefined,

      (p.metadata as Record<string, any>) ?? {},

      p.cancellationReason ?? (p.metadata as any)?.cancellationReason ?? undefined,

      p.cancelledBy ?? (p.metadata as any)?.cancelledBy ?? undefined,
  ...
  static toPersistence(e: Order) {
    return {
      ...
      overweightDeliveryCharge: e.overweightDeliveryCharge,
      ...
      cancelledBy: e.cancelledBy ?? null,
