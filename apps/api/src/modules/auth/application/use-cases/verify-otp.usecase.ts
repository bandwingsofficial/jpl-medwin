  ...
  async execute(dto: VerifyOtpDto) {
    ...
    if (!identity) {
      const normalizedEmail = !isPhone
        ? this.profileDomainService.normalizeEmail(identifier)
        : undefined;

      const normalizedPhone = isPhone
        ? this.profileDomainService.normalizePhoneNumber(identifier)
        : undefined;

      const existingProfile = isPhone
        ? await this.profileRepo.findByPhoneNumber(normalizedPhone!)
        : await this.profileRepo.findByEmail(normalizedEmail!);

      if (existingProfile) {
        const existingUser = await this.userRepo.findById(existingProfile.userId);
        ...
        const userIdentities = await this.identityRepo.findActiveByUserId(user.id);
        const existingSameTypeIdentity = userIdentities.find(
          (i) => i.type === method,
        );

        if (existingSameTypeIdentity) {
          if (existingSameTypeIdentity.value !== identifier) {
            existingSameTypeIdentity.softDelete();
            await this.identityRepo.update(existingSameTypeIdentity);
            ...
            if (!existingSameTypeIdentity.isVerified) {
              existingSameTypeIdentity.verify();
              await this.identityRepo.update(existingSameTypeIdentity);
            }
            identity = existingSameTypeIdentity;
          }
        } else {
          identity = await this.identityRepo.create(
            new AuthIdentity(
        ...
        if (
          isPhone &&
          normalizedPhone &&
          existingProfile.phoneNumber !== normalizedPhone
        ) {
          existingProfile.updateDetails({ phoneNumber: normalizedPhone });
          profileUpdated = true;
        } else if (
          !isPhone &&
          normalizedEmail &&
          ...
          existingProfile.updateDetails({ email: normalizedEmail });
          profileUpdated = true;
        }
      ...
      // =======================
      // 🔄 ENSURE PROFILE EXISTS FOR EXISTING USER (BACKFILL/IDEMPOTENT)
      // =======================
      let existingProfile = await this.profileRepo.findByUserId(user.id);
      if (!existingProfile) {
        if (isPhone) {
          const normalizedPhone =
            this.profileDomainService.normalizePhoneNumber(identifier);
          existingProfile = await this.profileRepo.findByPhoneNumber(normalizedPhone);
        } else {
          const normalizedEmail =
            this.profileDomainService.normalizeEmail(identifier);
          existingProfile = await this.profileRepo.findByEmail(normalizedEmail);
        }
      }
      if (!existingProfile) {
        const userIdentities = await this.identityRepo.findActiveByUserId(user.id);
        ...
        const email = emailIdentity
          ? this.profileDomainService.normalizeEmail(emailIdentity.value)
          : !isPhone
          ? this.profileDomainService.normalizeEmail(identifier)
          : undefined;

        const phoneNumber = phoneIdentity
          ? this.profileDomainService.normalizePhoneNumber(phoneIdentity.value)
          : isPhone
          ? this.profileDomainService.normalizePhoneNumber(identifier)
          : undefined;
        ...
        if (
          ...
          !isPhone &&
          normalizedEmail &&
          (!existingProfile.email || existingProfile.email !== normalizedEmail)
        ) {
          existingProfile.updateDetails({ email: normalizedEmail });
          profileUpdated = true;
        }
