...
export class SessionMapper {
  ...
  static toDomain(session: PrismaSession): Session {
    return new Session(
      ...
      SessionMapper.toDomainPlatform(session.platform),
      SessionMapper.toDomainSessionType(session.type), // 🔥 FIXED
  ...
  static toPersistence(session: Session) {
    return {
      ...
      platform: SessionMapper.toPrismaPlatform(session.platform),

      // 🔥 FIXED
      type: SessionMapper.toPrismaSessionType(session.type),
