...
export interface BannerImageRepository {
  ...
  createWithSortOrder(image: BannerImage, requestedSortOrder?: number): Promise<BannerImage>;

  createMany(images: BannerImage[]): Promise<void>;

  update(image: BannerImage): Promise<BannerImage>;

  updateWithSortOrder(image: BannerImage, newSortOrder?: number): Promise<BannerImage>;
