...
input.customerType = parseCustomerType(
  String(input.customerType ?? ''),
    ...
    if (input.isOverweight) {
      const weight = Number(input.weightKg);
      if (
        input.weightKg === undefined ||
        input.weightKg === null ||
        input.weightKg === ('' as any) ||
        Number.isNaN(weight) ||
        weight <= 0
      ) {
        errors.push({
          field: 'weightKg',
          message: 'Product weight in KG is required and must be greater than zero for overweight products.',
        });
      }
    }
