...
const handleResetFilters =
    ...
    try {
      ...
      if (nextStatus === "INACTIVE") {
        try {
          ...
          showSuccess("Product Inactivated successfully");
        } catch (error: any) {
          const message = error?.response?.data?.message || "";
          if (message.includes("active variants")) {
            showConfirmToast(
              `${message}\n\nInactivate all variants also?`,
              async () => {
                try {
                  await toggleProductStatus.mutateAsync({
                    productId: product.id,
                  ...
                  showSuccess("Product and variants Inactivated");
                } catch (error: any) {
                  showError(error?.response?.data?.message || "Failed to Inactivate");
                }
              }
            );
            return;
  ...
  const handleDelete = async (product: any) => {
    if (product.status === "ACTIVE") {
      showWarning("Inactivate product before deleting");
      return;
    }
...
<div className="w-full bg-white rounded-xl border border-gray-100 overflow-visible">
        {products.length === 0 ? (
