...
import { showError, showSuccess, showConfirmToast, showWarning } from "@/shared/store/toast.store";

interface Props {
  data: SubCategory[];
  categories: Category[];
  onEdit: (item: SubCategory) => void;
  onDelete: (item: SubCategory) => Promise<void> | void;
  onToggleStatus: (item: SubCategory) => Promise<void> | void;
}
    ...
    <div className="space-y-4 w-full flex flex-col">
      ...
      <div className="flex flex-wrap items-center gap-3 bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          {/* Native select component styled to integrate perfectly without third-party module type errors */}
          ...
          <Table>
            ...
            <TableBody>
              {paginatedData.map((item) => {
                ...
                // 🔥 TOOLTIP MESSAGE
                const tooltipText = isParentInactive
                  ? "Activate main category first"
                  : item.status === "ACTIVE"
                  ? "Deactivate"
                  : "Activate";
                ...
                return (
                  <TableRow key={item.id}>
                    ...
                    {/* IMAGE */}
                    {/* 👉 FIX: Removed 'className' from TableCell since styles are shared globally */}
                    <TableCell>
                      {item.imageUrl ? (
                        <img
                          src={item.imageUrl}
                    ...
                    <TableCell>
                      <div className="flex justify-end items-center gap-1.5">
                        ...
                        <Button
                          variant="secondary"
                          size="icon"
                          disabled={isBusy || isParentInactive}
                          title={tooltipText}
                          ...
                          onClick={async () => {
                            ...
                            if (isParentInactive) {
                              showSuccess("Parent category is inactive");
                              return;
                            }
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              console.error(err);
                              showError("Delete failed, Delete first Mini Category");
                            } finally {
                              setDeletingId(null);
                            }
                          }}
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between p-4 border-t border-gray-100 bg-gray-50/50">
            <span className="text-xs font-medium text-gray-500">
              Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, filteredData.length)} of {filteredData.length} Sub Categories
            </span>
