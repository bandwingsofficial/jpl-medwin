...
export class ExpireCoinsUseCase {
  ...
  async execute() {
    const expiredTransactions =
      await this.transactionRepo.findExpiredTransactions();
      ...
      try {
        await this.prisma.$transaction(
          async (prismaTx) => {
            ...
            const coinsToExpire = Math.min(
              wallet.balance,
              tx.remainingCoins ?? tx.coins,
            );

            if (coinsToExpire <= 0) {
              await this.transactionRepo.markAsExpired(
                tx.id,
                prismaTx,
              );
              ...
              results.push({
                ...
                expiredCoins: 0,
                failedReason:
                  'No balance left to expire',
              });

              return;
            }

            const currentRemainingCoins =
              tx.remainingCoins ?? tx.coins;
