...
export class ProductGalleryService {
  ...
  async sync(
    ...
    if (input.mainImage !== undefined) {
      const currentMain = await this.imageRepo.findMainByProduct(product.id, tx);
      ...
      // CREATE / REPLACE MAIN
        ...
        if (currentMain) {
          ...
          if (currentMain.url !== url) {
            console.log('🔄 UPDATE PRODUCT MAIN:', currentMain.id);

            currentMain.url = url;

            await this.imageRepo.update(currentMain, tx);
          } else {
            console.log('⏭ PRODUCT MAIN UNCHANGED:', currentMain.id);
          }
        } else {
    ...
    let allImages = await this.imageRepo.findByProduct(product.id, false, tx);
    ...
    for (const image of existingGallery) {
      if (seen.has(image.url)) {
        console.log('🗑 REMOVE DUPLICATE PRODUCT IMAGE:', image.id);
    ...
    allImages = await this.imageRepo.findByProduct(product.id, false, tx);
    ...
    for (let i = 0; i < input.images.length; i++) {
      ...
      if (existing) {
        ...
        if (existing.alt !== alt || existing.sortOrder !== i) {
          console.log('🔄 UPDATE PRODUCT IMAGE:', existing.id);
          ...
          console.log('⏭ PRODUCT IMAGE UNCHANGED:', existing.id);
        }
      ...
      console.log('➕ CREATE PRODUCT IMAGE:', url);
    ...
    for (const image of existingGallery) {
      if (!importedUrls.has(image.url)) {
        console.log('🗑 REMOVE PRODUCT IMAGE:', image.id);
