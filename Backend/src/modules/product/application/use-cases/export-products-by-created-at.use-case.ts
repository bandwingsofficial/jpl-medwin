...
@Injectable()
export class ExportProductsByCreatedAtUseCase {
  constructor(
  ...
  async execute(fromDate: Date, toDate: Date) {
    ...
    const entities = await this.builder.buildByCreatedAt({
      fromDate,
      toDate,
    });
    ...
    return {
      success: true,
      message: 'Products exported successfully by created date.',
      summary: {
      ...
      file: {
        name: `products-created-at-${Date.now()}.xlsx`,
        mimeType:
